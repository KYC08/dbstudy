...
-- 9. 같은 부서 내에서 나보다 급여를 더 많이받는 사원을 조회하시오.
-- 관계
-- 나는 여러 사원과 관계를 맺는다.
-- 나(EMPLOYEES ME)     상대(EMPLOYEES YOU)
-- 같은 부서의 사원만 조인하기 위해서 부서번호로 조인조건을 생성함
SELECT ME.EMPLOYEE_ID  AS 사원번호
     , ME.LAST_NAME    AS 사원명
     , ME.SALARY       AS 급여
     , YOU.EMPLOYEE_ID AS 상대사원번호
     , YOU.LAST_NAME   AS 상대사원명
     , YOU.SALARY      AS 상대급여
...
SELECT E.EMPLOYEE_ID AS 사원번호
     , E.JOB_ID      AS 현재JOB_ID
     , H.JOB_ID      AS 예전JOB_ID
...
SELECT D.DEPARTMENT_ID, D.DEPARTMENT_NAME, L.STATE_PORVINCE, C.COUNTRY_NAME, R.REGION_NAME
